...
const KW = {
  ...
  urba: ["permis","urbanisme","aménagement","plu","foncier","domanialité","copropriété","habitat indigne","environnement","icpe","installation classée","éolien","photovolta"],
...
// === DIAGNOSTIC : teste chaque source et rapporte son état ===
async function fetchFeed(src){
  try{
    ...
    return { ok: items.length>0, reason: items.length>0 ? "" : "0 item (pas un flux ?)", items };
  }catch(e){ return { ok:false, reason: e.name==="AbortError"?"timeout":"injoignable", items:[] }; }
}
...
async function main(){
  ...
  console.log("=================================================");
  console.log("DIAGNOSTIC DE COLLECTE — "+all.length+" flux à tester");
  console.log("=================================================\n");
  ...
  for(let i=0;i<all.length;i+=BATCH){
    const chunk = await Promise.all(all.slice(i,i+BATCH).map(async src=>({src, res: await fetchFeed(src)})));
    for(const {src,res} of chunk){
      if(res.ok){ ok.push({nom:src.nom, n:res.items.length}); results.push(...res.items); }
      else { ko.push({nom:src.nom, reason:res.reason}); }
    }
  }

  console.log("----- ✓ FLUX QUI RÉPONDENT ("+ok.length+") -----");
  ok.sort((a,b)=>b.n-a.n).forEach(s=>console.log("  ✓ "+s.n+" items  ·  "+s.nom));
  console.log("\n----- ✗ FLUX MUETS / EN ÉCHEC ("+ko.length+") -----");
  ko.forEach(s=>console.log("  ✗ "+s.reason.padEnd(16)+"·  "+s.nom));
  console.log("\n=================================================");
  console.log("BILAN : "+ok.length+" flux OK / "+all.length+" testés  ·  "+results.length+" items bruts récoltés");
  console.log("=================================================\n");

  // dédoublonnage + tri + classement (comme en prod)
  const seen = new Set();
  ...
  const out = { generatedAt:new Date().toISOString(), sources:all.length, fluxOk:ok.length, count:items.length, items };
  writeFileSync(join(__dir,"..","veille.json"), JSON.stringify(out,null,2));
  console.log("✓ "+items.length+" actualités classées et écrites dans veille.json");
}
main();
